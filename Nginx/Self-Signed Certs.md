...
sudo openssl x509 -in ca.pem -purpose -noout -text
```
...
```bash
sudo openssl req -new -sha256 -subj "/CN=*.asicloud.local" -key privkey.pem -out cert.csr
```

3. Create a `extfile` with all the alternative names

```bash
sudo echo "subjectAltName=DNS:*.asicloud.local,IP:37.139.35.175" | sudo tee extfile.cnf
...
5. Create fullchain.pem

```bash
sudo cat cert.pem > fullchain.pem
sudo cat ca.pem >> fullchain.pem

# or
sudo cat cert.pem | sudo tee fullchain.pem
sudo cat ca.pem | sudo tee -a fullchain.pem
```
